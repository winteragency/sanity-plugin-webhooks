...
export function Webhook({webhookId, refreshInterval, webhookBodyComponent}: WebhookProps) {
  const apiVersion = '2023-06-21'
  const projectId = useProjectId()
  const client = useClient({apiVersion}).withConfig({requestTagPrefix: ''})

  const {isLoading, data}: SWRResponse<SanityWebhookAttempt[], Error> = useSWR(
  ...
  return (
    <Stack space={[3, 3, 4]}>
      ...
      {!isLoading &&
        data?.map((row) => (
          <WebhookAttempt key={row.id} attempt={row} webhookBodyComponent={webhookBodyComponent} />
        ))}
    </Stack>
  )
}
